...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/firefly5/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/firefly5/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    rospy.loginfo("Have published %s into %s!",position + velocity,'/firefly5/command/trajectory')
...
def callback(data):
    ...
    if action is None:
        ...
        if distance2target() > 1:
            ...
            rospy.loginfo("Firefly5 arrived target location!")
    elif action is not None:
        print("Firefly5 Should action:", action)
...
def publish_trajectory(data):
    ...
    firefly5_path_pub.publish(path)

def listener():
    rospy.Subscriber("/firefly5/ground_truth/odometry", Odometry, callback)
    rospy.Subscriber('/firefly5/ground_truth/odometry', Odometry, publish_trajectory)
    rospy.spin()

if __name__ == '__main__':
    rospy.init_node('firefly5_go2goal', anonymous=True)
    firefly5_path_pub = rospy.Publisher('/firefly5/path', Path, queue_size=10)
    firefly_command_publisher = rospy.Publisher('/firefly5/command/trajectory',MultiDOFJointTrajectory,queue_size=10)
    velocity_publisher = rospy.Publisher('/firefly5/velocity', Twist, queue_size = 10)
    R1 = Robot([10, 20, 4.5],[10, 0, 2], 2)
    ...
    if distance2initial() <= 1:
        rospy.loginfo("Firefly5 arrived initial location!")
