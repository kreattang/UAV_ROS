...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/firefly3/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/firefly3/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    rospy.loginfo("Have published %s into %s!",position + velocity,'/firefly3/command/trajectory')
...
def callback(data):
    ...
    if action is None:
        ...
        if distance2target() > 1:
            ...
            rospy.loginfo("Firefly3 arrived target location!")
    elif action is not None:
        print("Firefly3 Should action:", action)
...
def publish_trajectory(data):
    ...
    firefly3_path_pub.publish(path)

def listener():
    rospy.Subscriber("/firefly3/ground_truth/odometry", Odometry, callback)
    rospy.Subscriber('/firefly3/ground_truth/odometry', Odometry, publish_trajectory)
    rospy.spin()

if __name__ == '__main__':
    rospy.init_node('firefly3_go2goal', anonymous=True)
    firefly3_path_pub = rospy.Publisher('/firefly3/path', Path, queue_size=10)
    ...
    if distance2initial() <= 1:
        rospy.loginfo("Firefly3 arrived initial location!")
