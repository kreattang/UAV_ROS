...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/firefly7/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/firefly7/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    rospy.loginfo("Have published %s into %s!",position + velocity,'/firefly7/command/trajectory')
...
def callback(data):
    ...
    if action is None:
        ...
        if distance2target() > 1:
            ...
            rospy.loginfo("Firefly7 arrived target location!")
    elif action is not None:
        print("Firefly7 Should action:", action)
...
def publish_trajectory(data):
    ...
    firefly7_path_pub.publish(path)

def listener():
    rospy.Subscriber("/firefly7/ground_truth/odometry", Odometry, callback)
    rospy.Subscriber('/firefly7/ground_truth/odometry', Odometry, publish_trajectory)
    rospy.spin()

if __name__ == '__main__':
    rospy.init_node('firefly7_go2goal', anonymous=True)
    ...
    if distance2initial() <= 1:
        rospy.loginfo("Firefly7 arrived initial location!")
