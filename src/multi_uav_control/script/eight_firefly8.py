...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/firefly8/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/firefly8/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    rospy.loginfo("Have published %s into %s!",position + velocity,'/firefly8/command/trajectory')
...
def callback(data):
    ...
    if action is None:
        ...
        if distance2target() > 1:
            ...
            rospy.loginfo("Firefly8 arrived target location!")
    elif action is not None:
        print("Firefly8 Should action:", action)
...
def publish_trajectory(data):
    ...
    firefly8_path_pub.publish(path)

def listener():
    rospy.Subscriber("/firefly8/ground_truth/odometry", Odometry, callback)
    rospy.Subscriber('/firefly8/ground_truth/odometry', Odometry, publish_trajectory)
    rospy.spin()

if __name__ == '__main__':
    rospy.init_node('firefly8_go2goal', anonymous=True)
    firefly8_path_pub = rospy.Publisher('/firefly8/path', Path, queue_size=10)
    firefly_command_publisher = rospy.Publisher('/firefly8/command/trajectory',MultiDOFJointTrajectory,queue_size=10)
    velocity_publisher = rospy.Publisher('/firefly8/velocity', Twist, queue_size = 10)
    R1 = Robot([20, 10, 1],[0, 10, 2], 2)
