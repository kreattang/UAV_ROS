...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/iris/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/iris/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    iris_command_publisher.publish(traj)
    rospy.loginfo("Have published %s into %s!",position + velocity,'/iris/command/trajectory')
...
def callback(data):
    ...
    firefly = [I3.location_x, I3.location_y, I3.location_z, I3.velocity, I3.velocity_angle]
    action = collision_detecter(owner, [hummingbird, pelican, firefly])
    if action is None:
        ...
        if distance2target() > 1:
            ...
            print("Current angle:", R1.velocity_angle)
...
def listener():
    rospy.Subscriber("/iris/ground_truth/odometry", Odometry, callback)
    rospy.spin()

if __name__ == '__main__':
    rospy.init_node('iris_go2goal', anonymous=True)
    iris_command_publisher = rospy.Publisher('/iris/command/trajectory',MultiDOFJointTrajectory,queue_size=10)
    velocity_publisher = rospy.Publisher('/iris/velocity', Twist, queue_size = 10)
    R1 = Robot([10, 10, 3.5],[0, 0, 2], 0.6)
    ...
    if distance2initial() <= 1:
        rospy.loginfo("Iris arrived initial location!")
