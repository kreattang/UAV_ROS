...
class Robot():
    def __init__(self, location, target, velocity):
        ...
        self.pose_subscriber =  rospy.Subscriber("/pelican/ground_truth/odometry", Odometry, self.update_pose)
    def update_pose(self, data):
        ...
        self.velocity_publisher = rospy.Publisher('/pelican/velocity', Twist, queue_size = 10)
...
def publish_command(position,velocity):
    ...
    pelican_command_publisher.publish(traj)
    rospy.loginfo("Have published %s into %s!",position + velocity,'/pelican/command/trajectory')
...
def callback(data):
    ...
    hummingbird = [I2.location_x, I2.location_y, I2.location_z, I2.velocity, I2.velocity_angle]
    iris = [I3.location_x, I3.location_y, I3.location_z, I3.velocity, I3.velocity_angle]
    action = collision_detecter(owner, [firefly, hummingbird, iris])
    if action is None:
        ...
        if distance2target() > 1:
            ...
            rospy.loginfo("Pelican Arrived target location!")
    elif action is not None:
        print("Pelican Should action:", action)
...
if __name__ == '__main__':
    rospy.init_node('pelican_go2goal', anonymous=True)
    pelican_command_publisher = rospy.Publisher('/pelican/command/trajectory',MultiDOFJointTrajectory,queue_size=10)
    velocity_publisher = rospy.Publisher('/pelican/velocity', Twist, queue_size = 10)
    ...
    if distance2initial() <= 1:
        rospy.loginfo("Pelican Arrived initial location!")
        I1 = Intruder('firefly')
        I2 = Intruder('hummingbird')
        I3 = Intruder('iris')
        try:
            listener()
