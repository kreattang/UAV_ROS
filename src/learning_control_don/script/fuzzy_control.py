...
d_L = ctrl.Antecedent(np.arange(0, 4, 0.1), 'd_L')
d_L['N'] = fuzz.zmf(d_L.universe, 1.2, 4)
d_L['F'] = fuzz.smf(d_L.universe, 1.2, 4)

d_R = ctrl.Antecedent(np.arange(0,4, 0.1), 'd_R')
d_R['N'] = fuzz.zmf(d_R.universe, 1.2, 4)
d_R['F'] = fuzz.smf(d_R.universe, 1.2, 4)

d_F = ctrl.Antecedent(np.arange(0,4, 0.1), 'd_F')
d_F['N'] = fuzz.zmf(d_F.universe, 1.2, 4)
d_F['F'] = fuzz.smf(d_F.universe, 1.2, 4)
# d_R.view()
...
# One Direction
# L
rule1 = ctrl.Rule(d_L['F'], v['H'])
rule2 = ctrl.Rule(d_L['N'], v['H'])
rule2_1 = ctrl.Rule(d_L['N'], theta['S'])
FR_L = ctrl.ControlSystem([rule1, rule2, rule2_1])
FC_L = ctrl.ControlSystemSimulation(FR_L)

# F
rule3 = ctrl.Rule(d_F['F'], v['H'])
rule3_1 = ctrl.Rule(d_F['F'], theta['S'])
rule4 = ctrl.Rule(d_F['N'], v['H'])
rule5 = ctrl.Rule(d_F['N'], theta['M'])
FR_F = ctrl.ControlSystem([rule3, rule3_1, rule4, rule5])
FC_F = ctrl.ControlSystemSimulation(FR_F)

# R
rule6 = ctrl.Rule(d_R['F'], v['H'])
rule7 = ctrl.Rule(d_R['F'], theta['S'])
rule8 = ctrl.Rule(d_R['N'], v['L'])
rule9 = ctrl.Rule(d_R['N'], theta['M'])
FR_R = ctrl.ControlSystem([rule6, rule7, rule8, rule9])
...
rule15_1 = ctrl.Rule(d_R['F'] & d_F['F'], theta['S'])
rule16 = ctrl.Rule(d_R['F'] & d_F['N'], v['L'])
rule17 = ctrl.Rule(d_R['F'] & d_F['N'], theta['S'])
rule18 = ctrl.Rule(d_R['N'] & d_F['F'], v['L'])
rule19 = ctrl.Rule(d_R['N'] & d_F['F'], theta['M'])
rule20 = ctrl.Rule(d_R['N'] & d_F['N'], v['L'])
rule21 = ctrl.Rule(d_R['N'] & d_F['N'], theta['B'])
...
rule23 = ctrl.Rule(d_L['F'] & d_R['F'], theta['S'])
...
rule29 = ctrl.Rule(d_L['N'] & d_R['N'], theta['B'])
...
def Action(collision):
    ...
    if len(collision) == 1:
        ...
        Controller.input[arg1] = collision[0][1]
    if len(collision) == 2:
        ...
        Controller.input[arg1] = collision[0][1]
        Controller.input[arg2] = collision[1][1]
    if len(collision) == 3:
        Controller = FC_LRF
        arg1 = 'd_' + str(collision[0][0])
        arg2 = 'd_' + str(collision[1][0])
        ...
        Controller.input[arg1] = collision[0][1]
        Controller.input[arg2] = collision[1][1]
        Controller.input[arg3] = collision[2][1]
    Controller.compute()
